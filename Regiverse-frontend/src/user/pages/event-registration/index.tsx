...
const FoodScan: React.FC = () => {
  ...
  const { conferenceSlug } = useParams(); // Get the slug from the URL
  ...
  const handleMealClick = (day: number, meal: string) => {
    // ✅ Include /u/${conferenceSlug} in the path
    if (conferenceSlug) {
      navigate(`/u/${conferenceSlug}/food-scan/scan?day=${day}&meal=${meal}`);
    } else {
      // Fallback in case slug isn't found
