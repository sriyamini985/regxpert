      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl sm:rounded-[2.5rem] shadow-sm p-5 sm:p-8 md:p-10 border border-slate-200 h-fit">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-purple-500 rounded-xl sm:rounded-2xl flex items-center justify-center text-white text-lg sm:text-xl shadow-inner">✉️</div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Bulk Email Engine</h1>
              <p className="text-slate-500 font-medium text-xs sm:text-sm mt-1">Broadcast messages to targeted categories.</p>
            </div>
          </div>
 
          {/* BANNER IMAGE UPLOAD */}
          <div className="mb-6">
            <label className="block text-xs sm:text-sm font-bold text-slate-700 mb-2">Campaign Banner Image</label>
            {bannerImage ? (
              <div className="relative bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl overflow-hidden group">
                <img 
                  src={bannerImage} 
                  alt="Banner Preview" 
                ...
                <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                  <button 
                    ...
                    className="bg-rose-600 hover:bg-rose-700 text-white font-bold px-4 py-2 rounded-xl text-xs sm:text-sm transition-all shadow-md active:scale-95"
                  >
                    Remove Banner
                  </button>
                </div>
              ...
              <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 transition-all hover:border-purple-300">
                <label className="flex flex-col items-center justify-center cursor-pointer">
                  <div className="flex flex-col items-center justify-center pt-1 pb-2 sm:pt-2 sm:pb-3">
                    <span className="text-2xl sm:text-3xl mb-1 sm:mb-2">🖼️</span>
                    <p className="text-xs sm:text-sm font-bold text-slate-700">Add Campaign Banner</p>
                    <p className="text-[10px] sm:text-xs text-slate-400 mt-1">PNG, JPG, or JPEG (Max 2MB)</p>
                  </div>
                  <input 
                    type="file" 
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl mb-4 sm:mb-5 outline-none focus:ring-2 focus:ring-purple-500 text-sm sm:text-base font-medium transition-all text-slate-800"
          />
          ...
          <textarea
            ...
            rows={10}
            className="w-full bg-slate-50 border border-slate-200 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl mb-6 sm:mb-8 outline-none focus:ring-2 focus:ring-purple-500 text-sm sm:text-base font-medium transition-all resize-none text-slate-800"
          />
                ...
                return (
                  <label key={cat} className="flex items-center gap-2.5 p-2 sm:p-2.5 bg-slate-50 hover:bg-slate-100 rounded-xl cursor-pointer border border-slate-100 transition-all select-none">
                    <input 
                      ...
                      className="w-4 h-4 sm:w-5 h-5 rounded border-slate-300 text-purple-600 focus:ring-purple-500 cursor-pointer"
                    />
                    <div className="flex-1 flex justify-between items-center text-xs sm:text-sm">
                      <span className="font-bold text-slate-700">{cat}</span>
                      <span className="px-2 py-0.5 bg-purple-50 text-purple-700 rounded text-[10px] sm:text-xs font-black border border-purple-100">
                        {count}
                      </span>
                    </div>
                  </label>
          ...
          <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-sm p-5 sm:p-6 border border-slate-200 flex flex-col h-[300px] sm:h-[400px]">
            <div className="mb-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 bg-purple-50 text-purple-700 rounded-lg text-xs sm:text-sm font-bold border border-purple-100">
            ...
            {fetching ? (
              <div className="flex-1 flex items-center justify-center text-purple-500 font-bold text-xs sm:text-sm animate-pulse">Loading Database...</div>
            ) : (
                ...
                {filteredParticipants.map((p: any) => (
                  <div key={p._id} className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 sm:p-3 flex justify-between items-center">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[9px] sm:text-[10px] font-medium text-slate-500 truncate mt-0.5">{p.email || "No email on file"}</p>
                    </div>
