...
import { Mail, Lock, Eye, EyeOff, Check, Loader2, ArrowRight, ArrowLeft, Building2 } from "lucide-react";
...
export default function UserLogin() {
  ...
  // Load Remembered Credentials & Conferences
  useEffect(() => {
    const savedEmail = localStorage.getItem("rememberedEmail");
    const savedPassword = localStorage.getItem("rememberedPassword");
    const savedSlug = localStorage.getItem("rememberedSlug");
    ...
    fetch(`${API_URL}/api/conferences`)
      ...
      .catch((err) => {
        ...
        setConferenceError(err.message || "Failed to load active workspaces");
      })
    ...
    try {
      ...
      if (res.success) {
        ...
        if (rememberMe) {
          ...
          localStorage.setItem("rememberedSlug", selectedSlug);
              ...
              {forgotSuccess ? (
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-500 text-white rounded-xl py-2.5 text-sm font-semibold transition-colors"
                  >
                    Close
                  </button>
                </div>
                ...
                <form onSubmit={handleForgotSubmit} className="space-y-4">
                  <p className="text-slate-400 text-xs leading-relaxed">
                    Provide the email associated with your staff profile to retrieve a password recovery link.
                  </p>
                  <div>
                    <input
                      type="email"
                      ...
                      required
                      className="w-full bg-slate-950 border border-slate-800 text-slate-100 placeholder-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="w-1/2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl py-2.5 text-sm font-semibold flex items-center justify-center gap-2 transition-colors disabled:bg-blue-800"
                    >
