...
const ConferenceImport = () => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const slug = conferenceName.toLowerCase().replace(/\s+/g, "") + year;

      const formData = new FormData();
      // Changed "title" to "name" to fix MongoDB E11000 null error
      formData.append("name", conferenceName);
      formData.append("slug", slug);
      formData.append("file", excelFile);

      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/conferences/import`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-28 px-4">
      <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-xl p-10">
        <h1 className="text-5xl font-bold mb-3">Import Conference</h1>
        <p className="text-gray-500 mb-10 text-lg">Upload conference delegates excel</p>
        <div className="mb-8">
          <label className="block mb-3 font-semibold text-lg">Conference Name</label>
          <input
            type="text"
        ...
        <div className="mb-10">
          <label className="block mb-3 font-semibold text-lg">Upload Excel</label>
          <input
            type="file"
            accept=".xlsx,.xls,.csv"
