...
const CheckInStation = () => {
  ...
  const startCameraScanner = () => {
    ...
    setTimeout(() => {
      try {
        const html5QrCode = new Html5Qrcode("reader-checkin");
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const tempScanner = new Html5Qrcode("reader-checkin");
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow p-6 border border-slate-100">
            ...
            <div className={`max-w-md mx-auto overflow-hidden rounded-xl border border-slate-200 bg-slate-950 ${cameraStarted ? "block my-4" : "hidden"}`}>
              <div id="reader-checkin" className="w-full aspect-square" />
            </div>
              ...
              {!cameraStarted ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl active:scale-95 transition-all text-sm font-bold shadow-md shadow-blue-500/15"
                  >
                    📷 Start Camera
                  </button>
                  <button
